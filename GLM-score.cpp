 ...
 * https://raw.githubusercontent.com/Klab-Bioinfo-Tools/GLM-Score/master/GLM-Score_0.15.2.cpp
 */
int hydrogen_B = 0;
int lines_total = 0;
int lines_total2 = 0;
...
N(H2)H		lis				N
his, try N(H)H		arg				OCO
glutamic, aspartic acid NH		arg				CO
glu, asp NH		trp, pro, his	OH
tyr, thr, ser
...
void surface_tension_hydrophobicity_calculator(map<string, double> hydro_map,
                                               map<string, double> tension_map,
                                               string infile_name) {
  ifstream infile;
  ...
  while (getline(infile, p_dist_line)) {
    ...
    // cout << "v1 " << v[1] << endl;
    exists = find(aa_numbers.begin(), aa_numbers.end(), v[4].c_str()) !=
             aa_numbers.end();
    aa_numbers.push_back(v[4]);
    counter++;
    if (exists == 0) {
      hydrophobicity += hydro_map[v[2]];
      // cout << "v2\t" << v[2] << "\tvalue\t" << hydro_map[v[2]] << "\taanum\t"
      // << v[4] << endl;
...
int open_ligand() {
  ...
  if (!inlig) {
    cout << "Error opening input file...\n\nPlease, be sure that the *.PDB "
            "file you want to open is placed in the same\ndirectory of this "
            "program, that you wrote the complete and correct name and\n.pdb "
            "extention, and try again.\n"
         << endl;
    system("pause");
    exit(1);
  }
  ...
  outlig << endl; // remove this once I fix the bug that makes it necessary to
                  // have an extra new line when reading the ligand input.
  if (achou_mol2 == 0) {
    cout
        << "Input error: " << ligand_name
        << " doesn't look to be a *.mol2 file. "
        << "please check input files and order of input data in comand line.\n";
    system("pause");
    exit(1);
  }
  inlig.close();
...
void distancia() {
  ...
  int counter;
  ifstream protmp;
  protmp.open("protein.tmp"); // vai abrir o arquivo temporario da proteina para
                              // poder encontrar a raiz do doador ou aceitador e
                              // doador ou aceitador
  ofstream RT_found;
  ...
  while (!ligtmp.eof()) {
    ...
    if (hibr.compare(0, 3, "N.1") == 0 || hibr.compare(0, 3, "N.2") == 0 ||
        hibr.compare(0, 4, "N.ar") == 0 || hibr.compare(0, 5, "N.pl3") == 0) {
      r2 = 1.86;
      found_hibr = 1;
      // cout << hibr << " " << r2 << endl;
    }
    ...
    if (hibr.compare(0, 3, "O.2") == 0 ||
        hibr.compare(0, 5, "O.co2") == 0) { // check the radius of C.co2
      r2 = 1.66;
      found_hibr = 1;
    ...
    //--------LIGANTE-------------RADII-Raio-de-VDW-PERMITIDOS
    // cout << "*" << atomo << "*" << espaco_1 << "*" << nome_atomo << "*" <<
    // espaco_2 << "*" << espaco_3 << "*" << x << "*" << y << "*" << z << "*" <<
    // espaco_5 << "*" << hibr << "*" << enter; cout << endl; cout << "number: "
    // << num_x << "name: " << v[2] << endl;
    double DIS2 = 0;
    string linep;
    while (!protmp.eof()) {
      ...
      // cout << "*" << p_atomo << "*" << p_nome_atomo << "*" << p_aminoacido <<
      // "*" << p_num_aminoacido << "*" << px << "*" << py << "*" << pz << "*"
      // << p_enter;
      if (linep.compare(0, 1, "") == 0 || linep.compare(0, 1, "\n") == 0 ||
          linep.compare(0, 1, "\0") == 0) {
        break;
      }
      // cout << endl;
      ...
      // cout << num_pz << endl;
      // double DDA =
      // sqrt(((num_x-num_px)*(num_x-num_px))+((num_y-num_py)*(num_y-num_py))+((num_z-num_pz)+(num_z-num_pz)));
      // //distancia entre o atomo do doador e aceitador angulos cos-1=[(AB)2 +
      // (AC)2 - (BC)2 / 2(AB)*(AC)]
      ...
      if (p_nome_atomo.compare(0, 2, "CA") == 0 ||
          p_nome_atomo.compare(0, 2, "CB") == 0 ||
          p_nome_atomo.compare(0, 2, "CD") == 0 ||
          p_nome_atomo.compare(0, 2, "CE") == 0 ||
          p_nome_atomo.compare(0, 2, "CG") == 0 ||
          p_nome_atomo.compare(0, 2, "CH") == 0 ||
          p_nome_atomo.compare(0, 2, "CZ") == 0) {
        r1 = 1.87;
        found_p = 2;
      }
      /*
      ...
      */
      ...
      double extended = 0.7;
      // RT
      if (DIS2 <= (permitido + extended) &&
          nome_atomo.compare(0, 1, "H") != 0 && found_hibr == 1 &&
          found_p == 1) {
        // cout << DIS2 << " " << p_nome_atomo << " " << atomo << " " << hibr <<
        // " r1 " << r1 << " r2 " << r2 << endl; cout << atomo[1] << atomo[2] <<
        // atomo[3] << atomo[4] << atomo[5] << atomo[6] << endl;
        for (i = 0; i < (6 - atomo.size()); i++) {
          RT_found << " ";
        }
        RT_found << atomo << endl;
      }
      // VDW
      if (hibr.compare(0, 1, "H") != 0 && atomo.compare(0, 1, "H") != 0 &&
          found_hibr == 1 && found_p != 0) {
        // funcao para o VDW
      ...
      } // else{cout << "excluded: " << hibr << ", " << nome_atomo << endl;}//if
        // hibr 0 != H
      // HC
      double HC = 0;
      double HC2 = 0;
      double HC_exp = 0;
      if (p_nome_atomo.compare(0, 1, "C") == 0 &&
          nome_atomo.compare(0, 1, "C") == 0 && found_hibr == 1 &&
          found_p != 0) {
        ...
        if ((DIS2 > HC_allowed_1) && (DIS2 <= HC_allowed_2)) {
          ...
          // cout << "HC_total: " << HC_total2 << " HC2: " << HC2 << "
          // perimitido1: " << permitido1 << " r1: " << r1 << " r2: " << r2 <<
          // endl; cout << "*" << HC << endl;
        }
      ...
      if (DIS2 <= permitido && nome_atomo.compare(0, 1, "H") != 0 &&
          found_hibr == 1) {
        repulsive++;
      ...
      //---------------------HC2--------------------------------
      if (/*DIS2 <= permitido1 ||*/ DIS2 >= permitido1 && DIS2 <= permitido &&
          found_hibr == 1 && found_p == 1) {
        if ((p_nome_atomo.compare(0, 1, "N") == 0 ||
             p_nome_atomo.compare(0, 1, "O") == 0 ||
             p_nome_atomo.compare(0, 1, "S") == 0) &&
            (nome_atomo.compare(0, 1, "N") == 0 ||
             nome_atomo.compare(0, 1, "O") == 0 ||
             nome_atomo.compare(0, 1, "S") == 0)) {
          ...
          n = 0;
          for (n = 0; n < (7 - atomo.size()); n++) {
            ldistmp << " ";
          }
          ldistmp << atomo << "  " << nome_atomo;
          for (n = 0; n < (7 - nome_atomo.size()); n++) {
            ldistmp << " ";
          }
          for (n = 0; n < (10 - x.size()); n++) {
            ldistmp << " ";
          }
          ldistmp << x;
          for (n = 0; n < (10 - y.size()); n++) {
            ldistmp << " ";
          }
          ldistmp << y;
          for (n = 0; n < (10 - z.size()); n++) {
            ldistmp << " ";
          }
          ldistmp << z << " " << hibr;
          for (n = 0; n < (5 - hibr.size()); n++) {
            ldistmp << " ";
          }
          ldistmp << endl;
          n = 0;
          ...
          if (p_atomo.size() < 6) {
            for (n = 0; n < (6 - p_atomo.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << p_atomo << " " << p_nome_atomo;
          if (p_nome_atomo.size() < 4) {
            for (n = 0; n < (4 - p_nome_atomo.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << p_aminoacido;
          if (p_aminoacido.size() < 4) {
            for (n = 0; n < (4 - p_aminoacido.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << chain;
          if (p_num_aminoacido.size() < 4) {
            for (n = 0; n < (4 - p_num_aminoacido.size()); n++) {
              pdistmp << " ";
            }
          }
          if (p_num_aminoacido.size() >= 4 && p_num_aminoacido.size() < 9) {
            for (n = 0; n < (9 - p_num_aminoacido.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << p_num_aminoacido;
          if (p_x.size() < 9) {
            for (n = 0; n < (9 - p_x.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << p_x;
          if (p_y.size() <= 8) {
            for (n = 0; n < (8 - p_y.size()); n++) {
              pdistmp << " ";
            }
          }
          pdistmp << p_y;
          if (p_z.size() <= 8) {
            for (n = 0; n < (8 - p_z.size()); n++) {
              pdistmp << " ";
            }
            pdistmp << p_z << " \n";
          } else {
...
int open_protein() {
  ...
  if (achou_pdb == 0) {
    cout
        << "Input error: " << protein_name
        << " doesn't look to be a *.pdb file. "
        << "please check input files and order of input data in comand line.\n";
    system("pause");
    exit(1);
  }
...
void raiz_proteina() {
  ...
  while (!AD.eof()) {
    ...
    // cout << "1" << pp_atomo <<  "2" << pp_nome_atomo <<  "3" << pp_aminoacido
    // <<  "4" << pp_num_aminoacido <<  "5" << ppx <<  "6" << ppy <<  "7" << ppz
    // <<  "8" << pp_enter << endl; cout << pp_nome_atomo[0] << pp_nome_atomo[1]
    // << pp_nome_atomo[2] << endl;
    ifstream AD_root;
    ...
    while (!AD_root.eof()) {
      ...
      // cout << "1" << p_atomo <<  "2" << p_nome_atomo <<  "3" << p_aminoacido
      // <<  "4" << p_num_aminoacido <<  "5" << px <<  "6" << py <<  "7" << pz
      // <<  "8" << p_enter << endl;
      //-------------------------------N-PROLINA
      if (pp_nome_atomo[0] == 'N' && pp_aminoacido[0] == 'P' &&
          pp_aminoacido[1] == 'R' && pp_aminoacido[2] == 'O') {
        if (aa != 1) {
          num_p_num_aminoacido = atoi(p_num_aminoacido);
          num_pp_num_aminoacido = atoi(pp_num_aminoacido);
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == ' ' &&
              num_p_num_aminoacido == (num_pp_num_aminoacido - 1)) {
            // cout << "R" << p_atomo << "*" << p_nome_atomo << "*" <<
            // p_aminoacido << "*" << p_num_aminoacido <<  "*" << px <<  "*" <<
            // py << "*" << pz << "*" << p_enter;
            //------cria arquivo temp com as raizes
            root << "R" << p_atomo << p_nome_atomo << p_aminoacido
                 << p_num_aminoacido << px << py << pz << p_enter;
          } // num_p_num_aminoacido==(num_pp_num_aminoacido-1))
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'A' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
              pp_num_aminoacido[1] == p_num_aminoacido[1] &&
              pp_num_aminoacido[2] == p_num_aminoacido[2] &&
              pp_num_aminoacido[3] == p_num_aminoacido[3] &&
              pp_num_aminoacido[4] == p_num_aminoacido[4] &&
              pp_num_aminoacido[5] == p_num_aminoacido[5] &&
              pp_num_aminoacido[6] == p_num_aminoacido[6] &&
          ...
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'D' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
        ...
        if (aa == 1) // se nao for o primeiro aminoacido
        {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'A' &&
            ...
            root << "R" << p_atomo << p_nome_atomo << p_aminoacido
                 << p_num_aminoacido << px << py << pz << p_enter;
            // cout << "1" << p_atomo <<  "2" << p_nome_atomo <<  "3" <<
            // p_aminoacido <<  "4" << p_num_aminoacido <<  "5" << px <<  "6" <<
            // py <<  "7" << pz <<  "8" << p_enter << endl; cout << "1" <<
            // pp_atomo <<  "2" << pp_nome_atomo <<  "3" << pp_aminoacido << "4"
            // << pp_num_aminoacido <<  "5" << ppx <<  "6" << ppy <<  "7" << ppz
            // <<  "8" << pp_enter << endl;
          } // if ........
        }   // if (aa==1)
      }
      //----------------------------------N-TODAS-PROTEINAS-EXCETO-PRO
      if (pp_nome_atomo[0] == 'N' &&
          (pp_aminoacido[0] != 'P' || pp_aminoacido[1] != 'R' ||
           pp_aminoacido[2] != 'O')) {
        //----------------------------------N
        if (pp_nome_atomo[1] == ' ') {
          if (aa != 1) // se nao for o prmeiro aminoacido
          {
            ...
            if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == ' ' &&
                num_p_num_aminoacido == num_pp_num_aminoacido_2) {
              // cout << "R" << p_atomo << "*" << p_nome_atomo << "*" <<
              // p_aminoacido << "*" << p_num_aminoacido <<  "*" << px <<  "*"
              // << py << "*" << pz << "*" << p_enter << endl;
              root << "R" << p_atomo << p_nome_atomo << p_aminoacido
            ...
            if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'A' &&
                p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
                pp_aminoacido[1] == p_aminoacido[1] &&
                pp_aminoacido[2] == p_aminoacido[2] &&
                pp_num_aminoacido[0] == p_num_aminoacido[0] &&
                pp_num_aminoacido[1] == p_num_aminoacido[1] &&
          ...
          if (aa == 1) // se for o primeiro amino�ido
          {
            if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'A' &&
              ...
              root << "R" << p_atomo << p_nome_atomo << p_aminoacido
                   << p_num_aminoacido << px << py << pz << p_enter;
              // cout << "1" << pp_atomo <<  "2" << pp_nome_atomo <<  "3" <<
              // pp_aminoacido <<  "4" << pp_num_aminoacido <<  "5" << ppx <<
              // "6" << ppy <<  "7" << ppz <<  "8" << pp_enter << endl;
            } // if ........
          }   // if (aa==1)
        }     // if (pp_nome_atomo[1]==' ')
        //----------------------------------NZ-LYS
        if (pp_nome_atomo[1] == 'Z' && pp_nome_atomo[2] == ' ' &&
            pp_aminoacido[0] == 'L' && pp_aminoacido[1] == 'Y' &&
            pp_aminoacido[2] == 'S') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'E' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
        ...
        //----------------------------------NE-ARG
        if (pp_nome_atomo[1] == 'E' && pp_nome_atomo[2] == ' ' &&
            pp_aminoacido[0] == 'A' && pp_aminoacido[1] == 'R' &&
            pp_aminoacido[2] == 'G') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'D' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
          ...
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'Z' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
        ...
        if (pp_nome_atomo[1] == 'H' &&
            (pp_nome_atomo[2] == '1' || pp_nome_atomo[2] == '2') &&
            pp_aminoacido[0] == 'A' && pp_aminoacido[1] == 'R' &&
            pp_aminoacido[2] == 'G') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'Z' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
        ...
        //----------------------------------ND1-HIS
        if (pp_nome_atomo[1] == 'D' && pp_nome_atomo[2] == '1' &&
            pp_aminoacido[0] == 'H' && pp_aminoacido[1] == 'I' &&
            pp_aminoacido[2] == 'S') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'G' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
          ...
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'E' &&
              p_nome_atomo[2] == '1' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
              pp_num_aminoacido[1] == p_num_aminoacido[1] &&
        ...
        //----------------------------------NE2-HIS
        if (pp_nome_atomo[1] == 'E' && pp_nome_atomo[2] == '2' &&
            pp_aminoacido[0] == 'H' && pp_aminoacido[1] == 'I' &&
            pp_aminoacido[2] == 'S') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'D' &&
              p_nome_atomo[2] == '2' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
              pp_num_aminoacido[1] == p_num_aminoacido[1] &&
        ...
        //---------------------------ND2--ASN
        if (pp_nome_atomo[1] == 'D' && pp_nome_atomo[2] == '2' &&
            pp_aminoacido[0] == 'A' && pp_aminoacido[1] == 'S' &&
            pp_aminoacido[2] == 'N') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'G' &&
              p_nome_atomo[2] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
        ...
        //---------------------------NE1--GLN
        if (pp_nome_atomo[1] == 'E' && pp_nome_atomo[2] == '2' &&
            pp_aminoacido[0] == 'G' && pp_aminoacido[1] == 'L' &&
            pp_aminoacido[2] == 'N') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'D' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
        ...
        //---------------------------NE1--TRP
        if (pp_nome_atomo[1] == 'E' && pp_nome_atomo[2] == '1' &&
            pp_aminoacido[0] == 'T' && pp_aminoacido[1] == 'R' &&
            pp_aminoacido[2] == 'P') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'D' &&
              p_nome_atomo[2] == '1' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
          ...
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'E' &&
              p_nome_atomo[3] == '2' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
              pp_num_aminoacido[1] == p_num_aminoacido[1] &&
      ...
      if (pp_nome_atomo[0] == 'O') {
        ...
        //---------------------------OG--SER
        if (pp_nome_atomo[1] == 'G' && pp_nome_atomo[2] == ' ' &&
            pp_aminoacido[0] == 'S' && pp_aminoacido[1] == 'E' &&
            pp_aminoacido[2] == 'R') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'B' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
              pp_aminoacido[2] == p_aminoacido[2] &&
              pp_num_aminoacido[0] == p_num_aminoacido[0] &&
        ...
        //---------------------------OG1--THR
        if (pp_nome_atomo[1] == 'G' && pp_nome_atomo[2] == '1' &&
            pp_aminoacido[0] == 'T' && pp_aminoacido[1] == 'H' &&
            pp_aminoacido[2] == 'R') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'B' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
        ...
        //---------------------------OH--TYR
        if (pp_nome_atomo[1] == 'H' && pp_nome_atomo[2] == ' ' &&
            pp_aminoacido[0] == 'T' && pp_aminoacido[1] == 'Y' &&
            pp_aminoacido[2] == 'R') {
          if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'Z' &&
              p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
              pp_aminoacido[1] == p_aminoacido[1] &&
      ...
      //-------------------------------------S
      //--------------------------------SD-MET
      if (pp_nome_atomo[0] == 'S' && pp_nome_atomo[1] == 'D' &&
          pp_nome_atomo[2] == ' ' && pp_aminoacido[0] == 'M' &&
          pp_aminoacido[1] == 'E' && pp_aminoacido[2] == 'T') {
        if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'E' &&
            p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
            pp_aminoacido[1] == p_aminoacido[1] &&
            pp_aminoacido[2] == p_aminoacido[2] &&
            ...
            pp_num_aminoacido[7] == p_num_aminoacido[7]) {
          // cout << "R" << p_atomo << "*" << p_nome_atomo << "*" <<
          // p_aminoacido << "*" << p_num_aminoacido <<  "*" << px <<  "*" << py
          // << "*" << pz << "*" << p_enter;
          root << "R" << p_atomo << p_nome_atomo << p_aminoacido
               << p_num_aminoacido << px << py << pz << p_enter;
        } // if ........
      ...
      if (pp_nome_atomo[0] == 'S' &&
          (pp_nome_atomo[1] == ' ' || pp_nome_atomo[1] == 'G') &&
          pp_nome_atomo[2] == ' ' && pp_aminoacido[0] == 'C' &&
          pp_aminoacido[1] == 'Y' && pp_aminoacido[2] == 'S') {
        if (p_nome_atomo[0] == 'C' && p_nome_atomo[1] == 'B' &&
            p_nome_atomo[3] == ' ' && pp_aminoacido[0] == p_aminoacido[0] &&
            pp_aminoacido[1] == p_aminoacido[1] &&
            pp_aminoacido[2] == p_aminoacido[2] &&
    ...
    // cout << "B" << pp_atomo <<  "*" << pp_nome_atomo <<  "*" << pp_aminoacido
    // <<  "*" << pp_num_aminoacido <<  "*" << ppx <<  "*" << ppy <<  "*" << ppz
    // <<  "*" << pp_enter << endl;
    root << "B" << pp_atomo << pp_nome_atomo << pp_aminoacido
         << pp_num_aminoacido << ppx << ppy << ppz << pp_enter;
    AD_root.close();
  } // while (!AD.eof())
  AD.close();
  root.close();
...
void raiz_ligante() {
  ...
  rd:
    ...
    if (line[0] == '@' && line[1] == '<' && line[2] == 'T' && line[3] == 'R' &&
        line[4] == 'I' && line[5] == 'P' && line[6] == 'O' && line[7] == 'S' &&
        line[8] == '>' && line[9] != 'B' && line[10] != 'O' &&
        line[11] != 'N' && line[12] != 'D' && line[13] != '\n') {
      // pos = 0;
      goto rd;
    } //! BOND
    if (line[0] == '@' && line[1] == '<' && line[2] == 'T' && line[3] == 'R' &&
    ...
    point:
      while (ordem[0] != '@' && !LAD.eof()) {
        ...
        ordem[0] = ' ';
        while (ordem[0] == ' ' &&
               !LAD.eof()) { // LAD.get(ordem, sizeof(ordem),'\0');
          LAD.get(ordem[0]);
        }
        c1 = 0;
        c2 = 1;
        while (ordem[c1] != ' ' && ordem[c1] != '@' &&
               !LAD.eof()) { // LAD.get(ordem, sizeof(ordem),'\0');
          LAD.get(ordem[c2]);
          c1++;
          c2++;
        ...
        // cout << tipo << endl;
        if (ordem[0] != '@' && ordem[c1] != '@' && atomo_1[c1] != '@' &&
            atomo_2[c1] != '@' && atomo_1[c2] != '@' && atomo_2[c2] != '@') {
          num_ordem = atoi(ordem);
          num_atomo_1 = atoi(atomo_1);
          num_atomo_2 = atoi(atomo_2);
          // cout <<  "ordem*" << ordem  << "*ordem" << atomo_1 <<  "atomo1*" <<
          // atomo_2 <<  "atomo2*" << endl; cout << "numeros:" << endl; cout <<
          // "*" << num_ordem << "*" << " " << "*" << num_atomo_1 << "*" << " "
          // << "*" << num_atomo_2 << "*" << endl; bonds << num_atomo_1 <<
          // num_atomo_2 << endl;
          if (num_atomo_1 <= 9 && num_atomo_1 != 0) {
            bonds << "     " << num_atomo_1;
            bond_type << "     " << num_atomo_1 << endl;
          }
        ...
        if (ordem[0] == '@' || ordem[c1] == '@' || atomo_1[c1] == '@' ||
            atomo_2[c1] == '@' || ordem[c2] == '@' || atomo_1[c2] == '@' ||
            atomo_2[c2] == '@' || line[53] == '@') {
          while (!LAD.eof()) {
            LAD.get(line[0]);
          }
        }
        // cout << "ate aqui funcionou" << endl; //saida para ver se o programa
        // conseguiu ler os caracteres ate este ponto cout << atomo_2 << endl;
        // else {cout << "erro" << ordem << endl << atomo_1 << endl << atomo_2
        // << endl << endl;}
      }
  ...
  */
  ...
  get:
    ...
    if (tipo[0] == 'B' && conta_r == 1) {
      conta_r = 0;
      p_root_f << "R" << r1_atomo << r1_nome_atomo << r1_aminoacido
               << r1_num_aminoacido << r1x << r1y << r1z << r1_enter;
      p_root_f << "B" << p_atomo << p_nome_atomo << p_aminoacido
               << p_num_aminoacido << px << py << pz << p_enter;
      goto get;
    }
    ...
    if (tipo[0] == 'R' && conta_r == 0) {
      ...
      pos = 0;
      // cout << "*" << r1_atomo << "*" << r1_nome_atomo << "*" << r1_aminoacido
      // << "*" << r1_num_aminoacido << "*" << r1x << "*" << r1y << "*" << r1z
      // << "*" << r1_enter; cout << endl << conta_r << endl;
      goto get;
    } // if (tipo[0]=='R' && conta_r==0)
    // se a primeira linha for uma raiz
    if (tipo[0] == 'R' && conta_r == 1) {
      ...
      pos = 0;
      // cout << "*" << r2_atomo << "*" << r2_nome_atomo << "*" << r2_aminoacido
      // << "*" << r2_num_aminoacido << "*" << r2x << "*" << r2y << "*" << r2z
      // << "*" << r2_enter; cout << endl << conta_r << endl;
      goto get;
    } // if (tipo[0]=='R' && conta_r==0)
    if (tipo[0] == 'R' && conta_r == 2) {
      ...
      pos = 0;
      while (pos != 9) {
        r3_atomo[pos] = p_atomo[pos];
        pos++;
      }
      //------copia do nome do atomo
      pos = 0;
      while (pos != 6) {
        r3_nome_atomo[pos] = p_nome_atomo[pos];
        pos++;
      }
      //------copia do nome do aa
      pos = 0;
      while (pos != 5) {
        r3_aminoacido[pos] = p_aminoacido[pos];
        pos++;
      }
      //------copia do numero do atomo
      pos = 0;
      while (pos != 9) {
        r3_num_aminoacido[pos] = p_num_aminoacido[pos];
        pos++;
      }
      //------copia posicao x
      pos = 0;
      ...
      pos = 0;
      while (pos != 5) {
        r3_enter[pos] = p_enter[pos];
        pos++;
      }
      pos = 0;
      // cout << "*" << r3_atomo << "*" << r3_nome_atomo << "*" << r3_aminoacido
      // << "*" << r3_num_aminoacido << "*" << r3x << "*" << r3y << "*" << r3z
      // << "*" << r3_enter; cout << endl << conta_r << endl;
      goto get;
    } // if (tipo[0]=='R' && conta_r==0)
    ...
    if (tipo[0] == 'B' && (conta_r == 2 || conta_r == 3)) {
      ...
      if (conta_r == 2) {
        ...
        gcz2 = gcz2t / 2;
        // cout << "resultado(2): " << gcx3 << " / " << gcy3 << " / " << gcz3 <<
        // endl;
        p_root_f << "R   "
                 << "geom. center(2)   ";
        char letrasx[11];
        ...
        // cout << letrasx << endl;
        // cout << letrasx[0] << letrasx[1] << letrasx[2] << letrasx[3] <<
        // letrasx[4] << letrasx[5] << letrasx[6] << letrasx[7]; p_root_f <<
        // letrasx[0] << letrasx[1] << letrasx[2] << letrasx[3] << letrasx[4] <<
        // letrasx[5] << letrasx[6] << letrasx[7];
      ...
      if (conta_r == 3) {
        ...
        */
        ...
        p_root_f << letrasx << letrasy << letrasz;
        p_root_f << " " << endl;
        //  p_root_f << p_atomo << p_nome_atomo << p_aminoacido <<
        //  p_num_aminoacido << px << py << pz << p_enter;
        conta_r = 0;
      } // if (conta_r==3)
      p_root_f << tipo << p_atomo << p_nome_atomo << p_aminoacido
               << p_num_aminoacido << px << py << pz << p_enter;
    } // if (conta_r==2 || conta_r==3)
    if (tipo[0] != 'R' && tipo[0] != 'B' && tipo[0] != '\0') {
      cout << "ERRO! |" << tipo << "|" << endl;
    }
    // cout << tipo << p_atomo << p_nome_atomo << p_aminoacido <<
    // p_num_aminoacido << px << py << pz << p_enter;
  } // while (!p_root.eof())
  p_root.close();
  p_root_f.close();
...
void L_centro_geom() {
  ...
  */
  ...
  get:
    ...
    // cout << "*" << tipo << "*" << l_atomo << "*" << l_nome_atomo << "*" <<
    // l_resto << "*" << lx << "*" << ly << "*" << lz << "*" << l_enter; se nao
    // for raiz e tiver so uma raiz
    if (tipo[0] == 'B' && conta_r == 1) {
      conta_r = 0;
      l_root_f << "R" << r1_atomo << r1_nome_atomo << r1_resto << r1x << r1y
               << r1z << " " << endl;
      l_root_f << "B" << l_atomo << l_nome_atomo << l_resto << lx << ly << lz
               << " " << endl;
      goto get;
    }
    // se a primeira linha for uma raiz
    ...
    if (tipo[0] == 'B' && (conta_r == 2 || conta_r == 3)) {
      ...
      if (conta_r == 3) {
        ...
        l_root_f << letrasx << "  " << letrasy << "  " << letrasz;
        l_root_f << "  " << endl;
        //  p_root_f << l_atomo << l_nome_atomo << p_aminoacido <<
        //  p_num_aminoacido << lx << ly << lz << l_enter;
        conta_r = 0;
      } // if (conta_r==3)
      l_root_f << tipo << l_atomo << l_nome_atomo << l_resto << lx << ly << lz
               << " " << endl;
    } // if (conta_r==2 || conta_r==3)
    if (tipo[0] != 'R' && tipo[0] != 'B' && tipo[0] != '\0') {
      cout << "ERROR! Can't find ligand/protein atoms root! |" << tipo << "|"
           << endl;
    }
    // cout << tipo << l_atomo << l_nome_atomo << p_aminoacido <<
    // p_num_aminoacido << lx << ly << lz << l_enter;
  } // while (!p_root.eof())
  l_root.close();
  l_root_f.close();
...
void angulos() {
  ...
  double num_blz;
  char bl_enter[3];
  //------saida-arquivos-temporarios com ligante e proteina (atomos mais
  //proximos)
  ofstream p_result;
  ...
  while (!proteina.eof() || !ligante.eof()) {
    ...
    //	cout << "*" << p_tipo << "*" << p_atomo << "*" << p_nome_atomo << "*" <<
    //p_aminoacido << "*" << p_num_aminoacido << "*" << px << "*" << py << "*"
    //<< pz << "*" << p_enter;
    ...
    // cout << "*" << l_tipo << "*" << l_atomo << "*" << l_nome_atomo << "*" <<
    // l_resto << "*" << lx << "*" << ly << "*" << lz << "*" << l_enter;
    ...
    // cout << endl;
    //--------------- SALVA ARQUIVO PDB COM ATOMOS E RAIZES OS ATOMOS MAIS
    //PROXIMOS
    // int num1_rl_atomo = 0;
    int num1_bl_atomo = 0;
    // int num1_rp_atomo = 0;
    ...
    if ((angulo_1 >= 60) && (angulo_2 >= 60)) {
      ...
      l_result << "HETATM" << bl_atomo[2] << bl_atomo[3] << bl_atomo[4]
               << bl_atomo[5] << bl_atomo[6] << bl_atomo[7] << " "
               << bl_nome_atomo << "BLK" << bl_resto << "     " << c1_blx[0]
               << c1_blx[1] << c1_blx[2] << c1_blx[3] << c1_blx[4] << c1_blx[5]
               << c1_blx[6] << c1_blx[7] << c1_bly[0] << c1_bly[1] << c1_bly[2]
               << c1_bly[3] << c1_bly[4] << c1_bly[5] << c1_bly[6] << c1_bly[7]
               << c1_blz[0] << c1_blz[1] << c1_blz[2] << c1_blz[3] << c1_blz[4]
               << c1_blz[5] << c1_blz[6] << c1_blz[7]
               << "                         " << bl_enter;
      //}
      //------------------------------------------
      ...
      p_result << "ATOM " << bp_atomo << " " << bp_nome_atomo << bp_aminoacido
               << bp_num_aminoacido << "   " << c1_bpx << c1_bpy << c1_bpz
               << "                         " << bp_enter;
      //}
      //------------------------------------------
      char distance[14] = "             ";
  ...
  */
  ...
  bonds_log << "--------------------------------------------------------"
            << endl;
  bonds_log << "        TABLE: INTERMOLECULAR HYDROGEN BONDS            "
            << endl;
  bonds_log << endl;
  bonds_log << "       Protein               Ligand          Distance(A)"
            << endl;
  ifstream d_bonds;
  ...
  while (!p_bonds_final.eof() || !l_bonds_final.eof() || !d_bonds.eof()) {
    ...
    bonds_log << p_line[16] << p_line[17] << p_line[18] << p_line[19]
              << p_line[20];
    limit_p << p_line[16] << p_line[17] << p_line[18] << p_line[19]
            << p_line[20];
    // proteina numero aminoacido
    bonds_log << p_line[21] << p_line[22] << p_line[23] << p_line[24]
              << p_line[25] << p_line[26];
    limit_p << p_line[21] << p_line[22] << p_line[23] << p_line[24]
            << p_line[25] << p_line[26];
    // proteina nome atomo
    bonds_log << p_line[12] << p_line[13] << p_line[14] << p_line[15];
    limit_p << p_line[12] << p_line[13] << p_line[14] << p_line[15];
    // proteina numero atomo
    bonds_log << p_line[6] << p_line[7] << p_line[8] << p_line[9] << p_line[10]
              << p_line[11];
    limit_p << p_line[6] << p_line[7] << p_line[8] << p_line[9] << p_line[10]
            << p_line[11] << endl;
    // proteina divisoria
    bonds_log << "   ";
    //----------------------------------------------
    // ligante nome aminoacido
    bonds_log << l_line[16] << l_line[17] << l_line[18] << l_line[19]
              << l_line[20];
    limit_l << l_line[16] << l_line[17] << l_line[18] << l_line[19]
            << l_line[20];
    // ligante numero aminoacido
    // bonds_log << l_line[22] << l_line[23] << l_line[24] << l_line[25] <<
    // l_line[26]; ligante nome atomo
    bonds_log << l_line[12] << l_line[13] << l_line[14] << l_line[15];
    limit_l << l_line[12] << l_line[13] << l_line[14] << l_line[15];
    // ligante numero atomo
    bonds_log << l_line[6] << l_line[7] << l_line[8] << l_line[9] << l_line[10]
              << l_line[11];
    limit_l << l_line[6] << l_line[7] << l_line[8] << l_line[9] << l_line[10]
            << l_line[11] << endl;
    // liagante  divisoria
    bonds_log << "         ";
...
void saida_PDB() {
  ...
  while (!p_result.eof()) {
    ...
    // cout << "*" << ATOM << "*" << bp_atomo << "*" << esp1 << "*" <<
    // bp_nome_atomo << "*" << bp_aminoacido << "*" << bp_num_aminoacido << "*"
    // << esp2 << "*" << bpx << "*" << bpy << "*" << bpz << "*" << esp3 << "*"
    // << bp_enter;
    ...
    while (!protein.eof()) {
      ...
      if (num_p_num_aminoacido == num_bp_num_aminoacido /* && num_aminoacido_anterior!=num_p_num_aminoacido*/)
        ...
        if (num_p_atomo < num_bp_atomo) {
          result_PDB << "ATOM " << p_atomo << " " << esp1 << p_nome_atomo
                     << p_aminoacido << p_num_aminoacido << esp2 << px << py
                     << pz << esp3 << p_enter;
          // cout << "*" << "ATOM " << "*" << p_atomo << " " << "*" << esp1 <<
          // "*" << p_nome_atomo << "*" << p_aminoacido << "*" <<
          // p_num_aminoacido << "*" << esp2 << "*" << px << "*" << py << "*" <<
          // pz << "*" << esp3 << "*" << p_enter;
        }
        if (num_p_atomo == num_bp_atomo) {
          result_PDB << ATOM << bp_atomo << esp1 << bp_nome_atomo
                     << bp_aminoacido << bp_num_aminoacido << esp2 << bpx << bpy
                     << bpz << esp3 << bp_enter;
          // cout << "*" << ATOM << "*" << bp_atomo << "*" << esp1 << "*" <<
          // bp_nome_atomo << "*" << bp_aminoacido << "*" << bp_num_aminoacido
          // << "*" << esp2 << "*" << bpx << "*" << bpy << "*" << bpz << "*" <<
...
void salva_ligante() {
  ...
  while (!ligand.eof()) {
    ...
    if (!ligand.eof()) {
      l_result << "HETATM" << num_atomo << " " << nome_atomo << "BLK"
               << "          " << x << y << z << "                          "
               << "\n";
    }
    if (ligand.eof()) {
  ...
  while (!p_root.eof() && ((p_file_line - 1) < line_p_count)) {
    ...
    if ((p_root_line[0] == 'B') &&
        (((p_file_line - 1) != line_p_count) ||
         (p_root_line[2] != ch_p[line_p_count][15]) ||
         (p_root_line[3] != ch_p[line_p_count][16]) ||
         (p_root_line[4] != ch_p[line_p_count][17]) ||
         (p_root_line[5] != ch_p[line_p_count][18]) ||
         (p_root_line[6] != ch_p[line_p_count][19]))) {
      p_liged = 0;
    }
  }
  sprintf(p_liged_list[line_p_count], "%i", p_liged);
  // cout << "numero de atomos ligados ao" << ch_p[line_p_count] << " = " <<
  // p_liged << "*" << p_liged_list[line_p_count] << endl;
  p_liged = 0;
  line_p_count++;
  p_file_line = 0;
  p_root.clear();            // forget we hit the end of file
  p_root.seekg(0, ios::beg); // move to the start of the file
  if (line_p_count < p_TOTAL) {
    goto p_function;
  }
  p_root.close();
  //-------------- ABRE LIGANTE ---------------------
...
l_function:
  ...
  while (!l_root.eof() && ((l_file_line - 1) < line_l_count)) {
    ...
    if ((l_root_line[0] == 'R') && (l_root_line[48] != 'H') &&
        (l_root_line[48] != 'h')) {
      l_liged++;
    }
    if (l_root_line[0] == 'B') {
      l_file_line++; /*cout << "@" << l_file_line << "@";*/
    }
    if ((l_root_line[0] == 'B') &&
        (((l_file_line - 1) != line_l_count) ||
         (l_root_line[3] != ch_l[line_l_count][9]) ||
         (l_root_line[4] != ch_l[line_l_count][10]) ||
         (l_root_line[5] != ch_l[line_l_count][11]) ||
         (l_root_line[6] != ch_l[line_l_count][12]) ||
         (l_root_line[7] != ch_l[line_l_count][13]))) {
      l_liged = 0;
    }
  }
  sprintf(l_liged_list[line_l_count], "%i", l_liged);
  // cout << "numero de atomos ligados ao" << ch_l[line_l_count] << " = " <<
  // l_liged << "*" << l_liged_list[line_l_count] << endl;
  l_liged = 0;
  line_l_count++;
  l_file_line = 0;
  ...
  while (line_p_count < p_TOTAL) {
    ...
    while (string_line_count < p_TOTAL) {
      if ((ch_p[line_p_count][12] != 'X') &&
          (ch_p[line_p_count][12] == ch_p[string_line_count][12]) &&
          (ch_p[line_p_count][13] == ch_p[string_line_count][13]) &&
          (ch_p[line_p_count][14] == ch_p[string_line_count][14]) &&
          (ch_p[line_p_count][15] == ch_p[string_line_count][15]) &&
          (ch_p[line_p_count][16] == ch_p[string_line_count][16]) &&
          (ch_p[line_p_count][17] == ch_p[string_line_count][17]) &&
          (ch_p[line_p_count][18] == ch_p[string_line_count][18]) &&
          (ch_p[line_p_count][19] == ch_p[string_line_count][19])) {
        found++; // encontrou um atomo igual ao da string selecionada
        if (found > MAX) {
          ch_p[string_line_count][12] =
              'X'; // marca este atomo para excluir da procura
    ...
    string_line_count =
        0;     // reseta as linhas que vao serem comparadas com a selecionada
  ...
  while (line_l_count < l_TOTAL) {
    ...
    while (!limit_type.eof()) {
      ...
      // cout << type << "*" <<endl;
      if (type[5] == '2' && atom[1] == ch_l[line_l_count][9] &&
          atom[2] == ch_l[line_l_count][10] &&
          atom[3] == ch_l[line_l_count][11] &&
          atom[4] == ch_l[line_l_count][12] &&
          atom[5] == ch_l[line_l_count][13]) {
        MAX--;
    ...
    while (string_line_count < l_TOTAL) {
      if ((ch_l[line_l_count][9] != 'X') &&
          (ch_l[line_l_count][9] == ch_l[string_line_count][9]) &&
          (ch_l[line_l_count][10] == ch_l[string_line_count][10]) &&
          (ch_l[line_l_count][11] == ch_l[string_line_count][11]) &&
          (ch_l[line_l_count][12] == ch_l[string_line_count][12]) &&
          (ch_l[line_l_count][13] == ch_l[string_line_count][13])) {
        found++; // encontrou um atomo igual ao da string selecionada
        if (found > MAX) {
          ch_p[string_line_count][12] =
              'X'; // marca este atomo para excluir da procura
...
void calcula_RT() {
  ...
  // pega os numeros dos atomos que estao dentro do permitido para o RT e
  // elimina os repetidos da lista
  while (!RT_found.eof()) {
    RT_found.get(rt_line, sizeof(rt_line), '\0');
  ...
  while (!ligand.eof()) {
    ...
    atom_type[lig_line][0] = ligand_line[1];
    atom_type[lig_line][1] = ligand_line[2];
    atom_type[lig_line][2] = ligand_line[3];
    atom_type[lig_line][3] = ligand_line[4];
    atom_type[lig_line][4] = ligand_line[5];
    atom_type[lig_line][5] = ligand_line[6];
    atom_type[lig_line][6] = ligand_line[7];
    atom_type[lig_line][7] = ligand_line[47];
    atom_type[lig_line][8] = ligand_line[48];
    atom_type[lig_line][9] = ligand_line[49];
    atom_type[lig_line][10] = ligand_line[50];
    atom_type[lig_line][11] = ligand_line[51];
    lig_line++;
  }
  lig_line = 0;
  ligand.close();
  // ja carregou todos os atomos agora vai comecar a comparar e contar o numero
  // de ligacoes simples.
  int s_bond = 0;
  ...
  // cout << "***************";
  int sp2_count = 0;
  // inserir funcao para encontrar numero do atomo dentro da lista de permitidos
  // RT_string wf: i = rt_string.find(atom_0[current_line_0], 6);
  // if(i==string::npos)
  //{
  ...
  while (current_line_0 < lines_total2) {
    ...
    if (marker < line_count && current_line_0 < lines_total2) {
      while (marker < line_count) {
        ...
        if (!strncmp(rt_string[current_line_0], atom_1[marker], 6)) {
          atom_count++;
        }
        // se os atomos sao iguais e a ligacao eh simples
        if (!strncmp(rt_string[current_line_0], atom_1[marker], 6) &&
            atom_1[next][5] == '1') {
          // se o atomo for igual ao selecionado verifica se os dois atomos sao
          // sp3 ou sp2 copia somente o numero dos atomos para uma string
          ...
          if (sp2_count < 2 && mol2_type[num_atom2][7] != 'H' &&
              mol2_type[num_atom1][7] != 'H') {
            s_bond++;
          } // achou um rotor!!!!
            //	if (sp2_count > 1 && mol2_type[num_atom2][7]!='H') {cout << "2
            //sp2s";}
        }   // se os atomos sao iguais (encontrou um igual ao selecionado na
            // lista)
        marker += 2;
      }
    }
    ...
    */
    ...
    // cout << "Temporary RT: " << RT <<  " rotors: " << s_bond << " atom_count:
    // " << atom_count << endl;
    s_bond = 0;
    marker = 0;
    sp2_count = 0;
...
void result_score_calc() {
  ...
  // cout << HC_total2 << "\t" << VDW_total << "\t" << RT << "\t" << HB << "\t"
  // << hydrogen_B << "\t" << repulsive << london << endl; cout <<
  // "Hydrophobic_contact:         " << HC_total2 << endl; cout <<
  // "Van_der_waals:               " << VDW_total << endl; cout <<
  // "Deformation_effect:          " << RT << endl; cout <<
  // "Hydrogen_bonds_refined_(HB):         " << HB << endl; cout <<
  // "Hydrogen_bonds_non_filtered_(HB):         " << hydrogen_B << endl; cout <<
  // endl; cout <<
  // "---------------------------------------------------------------------------------"
  // << endl;
  ...
  // cout << "result: " << endl << result << "char" << protein_name << endl <<
  // ligand_pdb << endl;
  double ASA1 = 0;
  double ASA2 = 0;
  string sendme;
  ...
  char result3[10];
  char str4[100];
  if (strcmp(ligand_type, "DNA") == 0 || strcmp(ligand_type, "RNA") == 0 ||
      strcmp(ligand_type, "DNA/RNA") == 0 ||
      strcmp(ligand_type, "nucleotide") == 0 ||
      strcmp(ligand_type, "protein-DNA") == 0 ||
      strcmp(ligand_type, "protein-RNA") == 0 ||
      strcmp(ligand_type, "protein-DNA/RNA") == 0 ||
      strcmp(ligand_type, "protein-nucleotide") == 0 ||
      strcmp(ligand_type, "nucl") == 0) {
    strcpy(str4, "Rscript R/DNA/script.R ");
    strcpy(type, "DNA");
    // cout << "found DNA" << endl;
  } else {
    if (strcmp(ligand_type, "small") == 0 ||
        strcmp(ligand_type, "small molecule") == 0 ||
        strcmp(ligand_type, "drug") == 0 ||
        strcmp(ligand_type, "prtoein-small") == 0 ||
        strcmp(ligand_type, "protein-small molecule") == 0 ||
        strcmp(ligand_type, "protein-drug") == 0) {
      strcpy(str4, "Rscript R/small_molecule/script.R ");
      // cout << "found small molecule" << endl;
      strcpy(type, "small");
    } else {
      if (strcmp(ligand_type, "protein") == 0 ||
          strcmp(ligand_type, "protein-protein") == 0 ||
          strcmp(ligand_type, "peptide") == 0 ||
          strcmp(ligand_type, "polypetide") == 0 ||
          strcmp(ligand_type, "prot") == 0) {
        ...
        if (strcmp(ligand_type, "unknown") == 0) {
          cout << "No ligand type provided, using default scoring function "
                  "(protein-DNA/RNA).\n";
          strcpy(type, "DNA");
        } else {
          cout << "Found unknown ligand type parameter " << ligand_type
               << ", using default (DNA).\n";
          strcpy(type, "DNA");
        }
      }
  ...
  hydrophobicity = 0;
  surface_tension = 0;
  surface_tension_hydrophobicity_calculator(hydro_map, tension_map,
                                            protein_file_name);
  ...
  if (strcmp(type, "DNA") == 0) {
    // DNA
    interaction_terms << "V2\tV3\tV4\tV5\tV6\tV7\tV18\tV19\tV20\tV21\tV22\tV23"
                      << endl
                      << HC_total2 << "\t" << VDW_total << "\t" << RT << "\t"
                      << hydrogen_B << "\t" << ASA1 << "\t" << ASA2 << "\t"
                      << repulsive << "\t" << london << "\t"
                      << contact_hydrophobicity << "\t" << total_hydrophobicity
                      << "\t" << contact_surface_tension << "\t"
                      << total_surface_tension << endl;
  }
  if (strcmp(type, "small") == 0) {
    // small
    interaction_terms << "V2\tV3\tV4\tV5\tV6\tV7\tV17\tV18\tV20\tV21\tV22\tV23"
                      << endl
                      << HC_total2 << "\t" << VDW_total << "\t" << RT << "\t"
                      << hydrogen_B << "\t" << ASA1 << "\t" << ASA2 << "\t"
                      << repulsive << "\t" << london << "\t"
  ...
  if (strcmp(type, "protein") == 0) {
    // protein
    interaction_terms << "V2\tV3\tV4\tV5\tV6\tV7\tV16\tV17\tV20\tV21\tV22\tV23"
                      << endl
                      << HC_total2 << "\t" << VDW_total << "\t" << RT << "\t"
                      << hydrogen_B << "\t" << ASA1 << "\t" << ASA2 << "\t"
                      << repulsive << "\t" << london << "\t"
...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    cout << "usage: GLM-Score <protein_file PDB> <ligand_file PDB> "
            "<ligand_file MOL2> <Ligand Type> \n\n<Ligand Type> is the type of "
            "ligand molecule, which supports 3 alternative options:\n\nDNA (or "
            "RNA), small-molecule (or drug), and protein.\nIf no <Ligand Type> "
            "is provided, protein-DNA/RNA scoring function will be used as "
            "default.\n\n";
    exit(1);
  }
