...
public:
  ...
  const Atom &atom(std::string name) const {
    for (auto &&atom : *this) {
      if (atom.name == name)
        return atom;
    }
    ...
    if (!atoms.empty()) {
      ...
      residue.is_std =
          std::all_of(atoms.begin(), atoms.end(),
                      [](const Atom &atom) { return atom.is_std; });
...
Pdb read_pdb(const std::string &filename, Pdb &pdb) {
...
Vec<Mol2> read_mol2s(const std::string &fn) {
  std::string section;
  Vec<Mol2> ms;
  Mol2 *p = NULL;
